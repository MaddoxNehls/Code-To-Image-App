...
export const initialCode = `function convertTemperature(input) {
  ...
  const celsius = celsiusToFahrenheit(parsedInput);
  const fahrenheit = fahrenheitToCelsius(parsedInput);
  
  return \`\${parsedInput}°C is \${celsius.toFixed(2)}°F and \${parsedInput}°F is \${fahrenheit.toFixed(2)}°C.\`;
}

const temperatureInput = prompt("Enter the temperature (e.g., '35C' or '95F'):");
alert(convertTemperature(temperatureInput));`;
